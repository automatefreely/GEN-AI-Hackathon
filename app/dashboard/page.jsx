...
export default function Dashboard() {
    ...
    const [messages, setMessages] = useState([
        { role: 'ai', content: "Hello! I'm your AI tutor. What would you like to learn about your selected subject?" }
    ])
    const chatContainerRef = useRef(null)

    useEffect(() => {
        setIsFormValid(selectedSubject !== '' && learningStyle !== '')
    }, [selectedSubject, learningStyle])
                    ...
                    <motion.div
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <Select value={selectedSubject} onValueChange={setSelectedSubject}>
                                            <SelectTrigger
                                                id="subject"
                                                className={`w-full ${showValidationError && !selectedSubject ? 'border-red-500' : ''}`}>
                                        ...
                                        {showValidationError && !selectedSubject && (
                                            <p className="text-sm text-red-500">Please select a subject</p>
                                        )}
                                    </div>
                                    <div className="space-y-2">
                                        <label
                                        ...
                                        <Select value={learningStyle} onValueChange={setLearningStyle}>
                                            <SelectTrigger
                                                id="learning-style"
                                                className={`w-full ${showValidationError && !learningStyle ? 'border-red-500' : ''}`}>
                                        ...
                                        {showValidationError && !learningStyle && (
                                            <p className="text-sm text-red-500">Please select a learning style</p>
                                        )}
                                    </div>
                                </div>
                            </CardContent>
                        ...
                        <div className="flex justify-center">
                            <Link href='/learn'>
                                <Button
                                    size="lg"
                                    className={`rounded-full transition-all duration-300 ${isFormValid ? 'bg-primary hover:bg-primary-dark' : 'bg-gray-300 dark:bg-gray-700'
                                        }`}
